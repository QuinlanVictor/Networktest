...
note 1008 配合model09271007.py做修改，把输出尺度改为单一尺度
     1013 配合model修改
     1015 终于安好了tf，之后测试一下代码能否跑通
     1019 之前的测试让我知道，双尺度输出是可行的，但是单尺度输出会遇到某段代码的迭代问题，所以无法运行下去，要找出这个问题所在
     1021 还是没找出问题所在
     1024 再找找问题
     1111 仍然没有解决问题，但是更进了一步，不知道之后能不能解决
     1114 再来思考思考，感觉是改不好了
"""
...
def _main():
    ...
    # is_tiny_version = len(anchors)==7 # default setting 没删除tiny的代码，避免使用代码
    # if is_tiny_version:
...
def create_model(input_shape, anchors, num_classes, load_pretrained=True, freeze_body=2,
    ...
    # y_true = [Input(shape=(h // {0: 8}[l], w // {0: 8}[l], num_anchors , num_classes + 5)) for l in
    #           range(1)]
...
#     num_anchors = len(anchors)
#
#     y_true = [Input(shape=(h//{0:8}[l], w//{0:8}[l], \
