...
20201124  参考代码
          ...
          问题： SimpleITK的安装问题  已经解决，可以看看github上的记录
    ...
    for z in range(num_z):
        for x in range(num_x):
            for y in range(num_y):
                seg_block = seg_array[z * stridez:z * stridez + blockz, x * stridex:x * stridex + blockx,
...
def preprocess(blockzxy, config):
    ...
    for i in range(config['num_file']):  # num_file
        ...
        ct = sitk.ReadImage(os.path.join(config['file_path'], 'volume-' + str(i) + '.nii'),
                            sitk.sitkFloat32)  # sitk.sitkInt16 Read one image using SimpleITK
        origin = ct.GetOrigin()
        ...
        if config['window_wc']:
            ...
            print('window transform(', config['window_wc'][0], ',', config['window_wc'][1], '):', ct_array.min(),
                  ct_array.max())
        ...
        if ct_array.shape[0] < blockzxy[0]:
            ...
            config['saved_idx'] = generate_subimage(ct_array, seg_array,
                                                    config['stridezxy'][0], config['stridezxy'][1],
                                                    config['stridezxy'][2],
