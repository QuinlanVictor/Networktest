...
note 1103 测试双尺度的训练
     ...
     1117 对应model1117的训练文件，在train1114文件上进行修改，以进行网络结构训练的测试
          加入了标签平滑
     1118 修改了一下冻结网络层数那边的代码，昨晚的训练可能是因为这里没有修改导致的错误，今晚再排查一下
          还有就是data_generator那里的random=True的设置，还需要考虑一下
     1119 昨晚看了看训练的程序代码，应该就是冻结层数那里疏忽了，没有修改好，现在已经修改完毕
"""
...
def create_model(input_shape, anchors, num_classes,  freeze_body=True):
    ...
    y_true = [Input(shape=(h//{0:16,1:8}[l], w//{0:16,1:8}[l], num_anchors//2, num_classes+5)) for l in range(2)]
    ...
    model_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
        arguments={'anchors': anchors, 'num_classes': num_classes, 'ignore_thresh': 0.5, 'label_smoothing': 0.1})(
...
def data_generator(annotation_lines, batch_size, input_shape, anchors, num_classes):
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            image, box = get_random_data(annotation_lines[i], input_shape, random=True)#random对于数据进行增强了，需要考虑下是不是还需要这么做
